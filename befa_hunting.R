...
setwd('~/Dropbox (HMS)/RagGroup Team Folder/TEDDY/befa/')

data=as_tibble(read.csv('normalized_teddy_alignments_subset.tsv',sep='\t')) %>% drop_na()
colnames(data)[1]='Run'
...
write.csv(out,'befa_analysis_output.csv')
...
out=do.call('rbind',out)
out$sequence_name=names
write.csv(out,'befa_analysis_output_young_patients.csv')
