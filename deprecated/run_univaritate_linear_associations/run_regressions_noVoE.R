...
if(!file.exists(paste(args[[3]],"/",'association_output_full_',args[[1]],args[[2]],sep=''))) {
  ...
  proportion_samples_with_zero_abundance_each_gene = number_samples_with_zero_abundance_each_gene/nrow(temp)
  genes_to_remove = which(proportion_samples_with_zero_abundance_each_gene >= 1- proportion_cutoff)
