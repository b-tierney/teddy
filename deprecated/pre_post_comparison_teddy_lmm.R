...
full_dataset=merge(metadata,cag_abundances,by.x='sampleID',by.y='rowname') %>% filter(seroconverted_ever==1)
...
for(cag in cagset){
  tryCatch({
    output=lmer(full_dataset[,cag]~seroconverted_ever+age+(1|subjectID),data=full_dataset)
...
write.csv(outputList_full,'teddy_association_outputs_pre_post.csv')
