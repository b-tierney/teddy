...
abundance_data=args[1]
metadata_file=args[2]
HLA_list=args[3]
condition_list = args[4]
output_dir = args[5]

d_small = fread(abundance_data,sep=",",header=TRUE,data.table=FALSE,nrow=1)
d_small = d_small[,-1]
...
if(has_extra_row == TRUE) {
  d = fread(abundance_data,sep=",",header=TRUE,data.table=FALSE,skip=1)
  d = d[,-1]
} else {
  d = fread(abundance_data,sep=",",header=TRUE,data.table=FALSE)
  d = d[,-1]
}
