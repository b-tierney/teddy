...
full_diabimmune_significant_ever <- full_diabimmune %>% filter(by<=.05 & term == 'seroconverted_ever') %>% select(CAG) %>% mutate_if(is.factor,as.character) 
full_teddy_significant_ever_validated <- full_teddy %>% filter(p.value<=.05 & term == 'seroconverted_ever') %>% select(CAG) %>% mutate_if(is.factor,as.character) %>% filter(CAG %in% full_diabimmune_significant_ever$CAG)

full_diabimmune_significant_at_sampling <- full_diabimmune_samponly %>% filter(by<=.05 & term == 'seroconverted_at_sampling') %>% select(CAG) %>% mutate_if(is.factor,as.character) 
full_teddy_significant_at_sampling_validated <- full_teddy %>% filter(p.value<=.05 & term == 'seroconverted_at_sampling') %>% select(CAG) %>% mutate_if(is.factor,as.character) %>% filter(CAG %in% full_diabimmune_significant_at_sampling$CAG)
