...
sum(jaccard_dist_upper==1) # 1569
mean(jaccard_dist_upper) # 0.008095909
...
create_specification_curve = function(mydf,output_file_name,include_sample_plot=FALSE) {
  ...
  library(circlize)
  color_vec = colorRamp2(breaks=c(min(mydf$sample_number),max(mydf$sample_number)), colors=c("white","black"))
...
mean(losso_cox_res_high_case_no_before_condition$sample_number)
sd(losso_cox_res_high_case_no_before_condition$sample_number)
pdf("sample_number_hist.pdf")
ggplot(losso_cox_res_high_case_no_before_condition, aes(x=sample_number)) + geom_histogram(color="black", fill="white") + theme_classic() + xlab("Sample Size")
dev.off()
...
AUC_prediction_res2 = lm(AUC ~ condition + baseline + HLA + proportion_train + model + feature_selection_method + microbiome_type + weighted + sample_number + features +  horizon_time,data=losso_cox_res_high_case_no_before_condition)
...
plot_model(AUC_prediction_res2, show.values = TRUE, value.offset = 0.3,sort.est=TRUE,dot.size=1) + theme_classic() + ggtitle("")
dev.off()
...
ggplot(losso_cox_res_high_case_alluvial_df_melted, aes(y=Freq,))
```
